...
class Flickr30kDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if not evaluate:
            ...
            self.noisy_transforms = transforms.Compose([
                    transforms.Resize((224, 224)),
                    transforms.ToTensor(),
                    transforms.Resize(256),
                    transforms.CenterCrop(224),
                    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
                    transforms.Lambda(lambda x: x +noise_amount* torch.randn_like(x)) # add random noise to the image
                ])
        if evaluate:
            self.transform = transforms.Compose(
                [
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        if self.image_transform:
            img1, img2 = self.image_transform(image)
            return (self.transform(image),self.noisy_transforms(image),img1,img2)
        else:
            return (
                self.transform(image),
                self.noisy_transforms(image)
            )
