...
class Logger:
    def __init__(self, log_file, resnet_save_file, optimizer, learning_rate, weight_decay, batch_size, Momentum, temperature, total_epochs ,reconstruction_tradeoff,contrastive_tradeoff,intra_projection_dim,hidden_dim,scheduler,layers_to_train,noise_amount):
        """
        ...
        self.message += f'total_epochs: {total_epochs}\n'
        self.message += f'reconstruction_tradeoff: {reconstruction_tradeoff}\n'
        self.message += f'contrastive_tradeoff: {contrastive_tradeoff}\n'
        self.message += f'intra_projection_dim: {intra_projection_dim}\n'
        self.message += f'hidden_dim: {hidden_dim}\n'
        self.message += f'scheduler: {scheduler}\n'
        self.message += f'layers_to_train: {layers_to_train}\n'
        self.message += f'noise_amount: {noise_amount}\n\n'

        with open(self.log_file, 'w') as f:
            f.write(self.message)
    def log(self, epoch, resnet_model, train_loss, test_loss, time_for_epoch):
        """
